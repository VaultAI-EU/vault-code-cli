...
Prevent opencode from reading dotenv files (like `.env` and `.env.local`) without blocking similarly-named files like `.envrc`:
...
export const EnvProtection = async ({ project, client, $, directory, worktree }) => {
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const basename = path.basename(output.args.filePath)
      const isDotenv = /^\.env(\.|$)/.test(basename)
      const allowed =
        basename.endsWith(".env.sample") || basename.endsWith(".env.example") || basename.endsWith(".env.template")

      if (isDotenv && !allowed) throw new Error("Do not read .env files")
    },
  }
}
```
