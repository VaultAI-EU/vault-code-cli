...
For safety, `read` blocks dotenv files by default (for example `.env`, `.env.local`, `.env.production`). Template files like `.env.example`, `.env.sample`, and `.env.template` are allowed, and `.envrc` is not treated as a dotenv file.
